    ...
    <header id="first">
        <div class="header-content">
            <div class="inner">
                ...
                {!! Form::open(['url' => 'companies', 'method' => 'get', 'class' => 'form-inline']) !!}
    ...
    <div class="container " style="background-color: #ffffff;">
    	<div class=col-md-3>
    		...
    		<div class="filterBlock">
                ...
                <a href="/companies#results">Show all</a><br>
    		</div>
...
if (!empty($_GET["ozip"]) && is_numeric($_GET["ozip"]) && !empty($_GET["distance"]) && is_numeric($_GET["distance"])) {
  ...
  $city_req_sql = $link->prepare("SELECT city,street,compName FROM companies");
  $old_records_sql = $link->prepare("SELECT * FROM distances WHERE zipcode = $ozip");
  $count_companies = $link->prepare("SELECT COUNT(compName) AS counted FROM companies");
...
<?php
$tags_dtb = $company['tags'];

$tags = explode(',', $tags_dtb);

?>

<a <a href="{{url('companies',$company->id)}}">
    		<div class="company">
    			
	    			<div class="col-md-3">
	    				<img <img src="{{asset('img/'.$company->image.'')}}" class="img-responsive" />
	    			</div>
...
<?php
    $company3 = DB::table('companies')->where('tags', 'LIKE', "%".$q_string."%")->get();
?>
...
@foreach($company3 as $company2)

<a <a href="{{url('companies',$company2->id)}}">
            <div class="company">
                
                    <div class="col-md-3">
                        <img s<img src="{{asset('img/'.$company2->image.'')}}" class="img-responsive" />
                    </div>

                    <div class="col-md-9">
                        <h2>{{ $company2->compName }}</h2>
                        <p>{{ $company2->title }}</p>
                        <span style="color: #000;">Tags:</span> 
